            ...
            try {
                foreach ($listRecords as $listaRecord):
                    ...
                    try {
                        ...
                        // Verifica si la petición tiene todos los atributos
                        if (sizeof($listDifAtriP) == 0) 
                        {
                            $listObjRels = array_diff(array_keys($record), $model->atributes());

                            //echo '<pre>';
                            //echo var_dump($listObjRels) . '<br>';
                            //echo '</pre>';

                            $listDiffRel = array_diff($listObjRels, $model->getListHasMany());
                            $pk = $model->getNamePk();

                            // Busca el usuario que se esta modificando
                            $model = Usuario000101::findOne($record[$pk[0]]);
                            if ($model) {

                                // Obtiene los atributos de la tabla usuario (no asi las relaciones)
                                foreach ($model->atributes() as $atributo)
                                {
                                    if ($atributo == "contrasena_usuario")
                                    {
                                        if ($record[$atributo] == "")
                                            unset($record[$atributo]);
                                        else
                                ...
                                // Valida los atributos de la tabla usuarios
                                if ($model->validate())
                                {
                                    // Actualiza el registro en la tabla usuario
                                    $model->update();
                                    $audi->insertAudi("update", $model->tableName(), $model->getPrimaryKey());

                                    //echo '<pre>';
                                    //echo var_dump($model->getListHasMany()) . '<br>';
                                    //echo '</pre>';


                                    ////////////////////////////////////////////////////////////////////////
                                    // [INICIO] Proceso de los datos relacionados al usuario en otras tablas
                                    ////////////////////////////////////////////////////////////////////////
                                    foreach ($listObjRels as $listObjRel):
                                        ...
                                        if (in_array($listObjRel2, $model->getListHasMany())) {
                                            ...
                                            if ($record[$listObjRel]) {
                                                ...
                                                /*echo '<pre>';
                                                echo "Campos con valores:";
                                                echo var_dump($listObjRel) . '<br>';
                                                echo var_dump($record[$listObjRel]) . '<br>';
                                                echo '</pre>';*/
                                                ...
                                                foreach ($record[$listObjRel] as $subCampo):
                                                    ...
                                                    if (substr($listObjRel2, -2) != "02") {
                                                        ...
                                                        if ($sw == 0) {
                                                            $objRel::deleteAll("fk_id_usuario = " . $model->getPrimaryKey());
                                                            $sw = 1;
                                                        }
                                                    }
                                                    ...
                                                    foreach ($objRel->atributes() as $campo):
                                                        if ($campo == 'fk_id_usuario')
                                                            $objRel->$campo = $record['id_usuario'];
                                                        else
                                    ...
                                    endforeach;
                                    ////////////////////////////////////////////////////////////////////////
                                    // [FIN] Proceso de los datos relacionados al usuario en otras tablas
                                    ////////////////////////////////////////////////////////////////////////


                                    
                                }
                                // Almacena los errores del registro que se intenta guardar
                                else
                                {
                        ...
                        // Si la petición tiene diferentes campos genera un error
                        else
                        {
