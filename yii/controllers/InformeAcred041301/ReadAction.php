...
namespace app\controllers\InformeAcred041301;
use yii\base\Action;
		...
		if ($error == "") {
			try {
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = InformeAcred041301::find()
                    							->distinct(true)
												->with('fkIdPeticion')
												->asArray()
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = InformeAcred041301::find()
                                				->distinct(true)
                                                ->with('fkIdPeticion')
                                                ->asArray()
												->joinWith('fkIdPeticion')
                        ...
                        if ($sw == 0) {
                        	...
                        	$models = InformeAcred041301::find()
                                                ...
                                                ->asArray()
							->joinWith('fkIdPeticion')
                                                ->where(['IN','id_informe_acred',$listSubQuery[0]])
                                                ->andWhere(['IN','id_informe_acred',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = InformeAcred041301::find()
                            					->distinct(true)
