...
namespace app\controllers\GestionTipoActividad050103;
use yii\base\Action;
...
class ReadAction extends Action
{
    public function run()
    {
		...
		$model = new GestionTipoActividad050103;
		$error="";
		...
		if ($error == "") {
			try {
				if (isset($_GET['sort'])) {
					...
					$propertySort	= "id_gestion_tipo_actividad";
					$directionSort	= "asc";
				}
                $condiQuery = "";
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = GestionTipoActividad050103::find()
                    							->distinct(true)
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = GestionTipoActividad050103::find()
                                				->distinct(true)
                                                ->with('fkIdGestion','fkIdTipoActividad')
                                                ->asArray()
												->joinWith('fkIdGestion')
                        ...
                        if ($sw == 0) {
							...
							->joinWith('fkIdGestion')
							->joinWith('fkIdTipoActividad')
                                                ->where(['IN','id_gestion_tipo_actividad',$listSubQuery[0]])
                                                ->andWhere(['IN','id_gestion_tipo_actividad',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = GestionTipoActividad050103::find()
                            					->distinct(true)
