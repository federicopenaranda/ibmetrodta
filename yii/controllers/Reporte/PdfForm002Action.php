...
class PdfForm002Action extends  Action
{
    public function run()
    {
		...
		$head = ['h1'=>utf8_encode('DIRECCIÓN TÉCNICA DE ACREDITACIÓN'),
				...
				'h3'=>utf8_encode('MANIFIESTO DE INTENCIÓN DE <br /> ACREDITACIÓN'),
    	...
    	if (isset($_GET['id_oec'])){
			...
			foreach ($models["oecContacto020003s"] as $cont)
			{
				if ($cont["representante_legal_oec_contacto"] == 1)
					$replegal = $cont;
				else
					$replegal = -1;
			}
			...
			$mpdf->WriteHTML($this->controller->renderPartial('pdfForm002',array('models'=>$models, 'replegal'=>$replegal, 'fecha'=>$fecha),true));
