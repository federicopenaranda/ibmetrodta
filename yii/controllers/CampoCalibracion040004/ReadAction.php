...
namespace app\controllers\CampoCalibracion040004;
use yii\base\Action;
...
class ReadAction extends Action
{
    public function run()
    {
		...
		$model = new CampoCalibracion040004;
		$error="";
		...
		if ($error == "") {
			try {
				if (isset($_GET['sort'])) {
					...
					$propertySort	= "id_campo_calibracion";
					$directionSort	= "asc";
				}
                $condiQuery = "";
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = CampoCalibracion040004::find()
                    							->distinct(true)
												->asArray()
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = CampoCalibracion040004::find()
                                				->distinct(true)
                                                ->asArray()
                        ...
                        if ($sw == 0) {
                        	...
                        	$models = CampoCalibracion040004::find()
                            					->distinct(true)
                                                ->asArray()
                                                ->where(['IN','id_campo_calibracion',$listSubQuery[0]])
                                                ->andWhere(['IN','id_campo_calibracion',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = CampoCalibracion040004::find()
                            					->distinct(true)
