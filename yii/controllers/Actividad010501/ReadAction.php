		...
		if ($error == "") {
			try {
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = Actividad010501::find()
                    							->distinct(true)
												->with('fkIdActividadTipo','fkIdProceso','actividadEstado010602s','actividadTransicion010502s')
												->asArray()
												->joinWith('fkIdActividadTipo')
												->joinWith('fkIdProceso')
												->joinWith('actividadEstado010602s')
												->joinWith('fkIdEstados')
					...
					$models = Actividad010501::find()
											...
											for ($i=0; $i < sizeof($models); $i++) {
                            					unset($models[$i]['fkIdActividadTipo']);
					...
					for ($i=0; $i < sizeof($models); $i++) {
						...
						for ($j=0; $j < sizeof($models[$i]['actividadEstado010602s']); $j++){
							$aux[] = $models[$i]['actividadEstado010602s'][$j]['fk_id_estado'];	
						}
					...
					for ($i=0; $i < sizeof($models); $i++) {
						...
						for ($j=0; $j < sizeof($models[$i]['actividadTransicion010502s']); $j++){
							$aux[] = $models[$i]['actividadTransicion010502s'][$j]['fk_id_transicion'];	
						}
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = Actividad010501::find()
                                				->distinct(true)
                                                ->with('fkIdActividadTipo','fkIdProceso','actividadEstado010602s','actividadTransicion010502s')
                                                ->asArray()
												->joinWith('fkIdActividadTipo')
