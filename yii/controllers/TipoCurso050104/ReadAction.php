...
namespace app\controllers\TipoCurso050104;
use yii\base\Action;
		...
		if ($error == "") {
			try {
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = TipoCurso050104::find()
                    							->distinct(true)
												->asArray()
												->joinWith('actividad050101s')
												->where($condiQuery)
                                                ->all();
                    $total = sizeof($models);
					$models = TipoCurso050104::find()
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = TipoCurso050104::find()
                                				->distinct(true)
                                                ->asArray()
                        ...
                        if ($sw == 0) {
                        	...
                        	$models = TipoCurso050104::find()
                            					->distinct(true)
                                                ->asArray()
                                                ->where(['IN','id_tipo_curso',$listSubQuery[0]])
                                                ->andWhere(['IN','id_tipo_curso',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = TipoCurso050104::find()
                            					->distinct(true)
