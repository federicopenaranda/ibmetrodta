    	        	...
    	        	try {
						...
						if (sizeof($listDifAtriP) == 0) {
							...
							$listDiffRel = array_diff($listObjRels,$model->getListHasMany());
                                                        $pk=$model->getNamePk();
							$model = PeticionAccion010301::findOne($record[$pk[0]]);
							if ($model) {
                                            ...
                                            if ($record[$listObjRel]){
                                                ...
                                                foreach ($record[$listObjRel] as $subCampo):
                                                    ...
                                                    if(substr($listObjRel2,-2) != "02"){
	                                                    ...
	                                                    if ($sw==0) {
	                                                        $objRel::deleteAll("fk_id_peticion_accion = ".$model->getPrimaryKey());
	                                                        $sw=1;
	                                                    }
                                                    }
                                                    foreach($objRel->atributes() as $campo):	
                                                        if ($campo == 'fk_id_peticion_accion')
                                                            $objRel->$campo=$record['id_peticion_accion'];
                                                        else {
