    	        	...
    	        	try {
						...
						if (sizeof($listDifAtriP) == 0) {
							...
							$listDiffRel = array_diff($listObjRels,$model->getListHasMany());
                                                        $pk=$model->getNamePk();
							$model = UsuarioOec000102::findOne($record[$pk[0]]);
							if ($model) {
                                            ...
                                            if ($record[$listObjRel]){
                                                ...
                                                foreach ($record[$listObjRel] as $subCampo):
                                                    ...
                                                    if(substr($listObjRel2,-2) != "02"){
	                                                    ...
	                                                    if ($sw==0) {
	                                                        $objRel::deleteAll("fk_fk_id_usuario = ".$model->getPrimaryKey());
	                                                        $sw=1;
	                                                    }
	                                                }
                                                    
                                                    foreach($objRel->atributes() as $campo):	
                                                        if ($campo == 'fk_fk_id_usuario')
                                                            $objRel->$campo=$record['fk_id_usuario'];
                                                        else {
