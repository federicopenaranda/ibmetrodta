    	        	...
    	        	try {
						...
						if (sizeof($listDifAtriP) == 0) {
							...
							$listDiffRel = array_diff($listObjRels,$model->getListHasMany());
                                                        $pk=$model->getNamePk();
							$model = ActividadUsuario050103::findOne($record[$pk[0]]);
							if ($model) {
                                        ...
                                        if (in_array($listObjRel2,$model->getListHasMany())) {		
                                            ...
                                            if ($record[$listObjRel]){
                                                ...
                                                foreach ($record[$listObjRel] as $subCampo):
                                                    ...
                                                    if ($sw==0) {
                                                        $objRel::deleteAll("fk_id_actividad_usuario = ".$model->getPrimaryKey());
                                                        $sw=1;
                                                    }
                                                    
                                                    foreach($objRel->atributes() as $campo):	
                                                        if ($campo == 'fk_id_actividad_usuario')
                                                            $objRel->$campo=$record['id_actividad_usuario'];
                                                        else {
