                    ...
                    try {
                        ...
                        if (sizeof($listDifAtriP) == 0) {
                            ...
                            foreach ($model->atributes() as $atributo) {
                                ...
                                /**
                                 * esta parte es para armar el envio de correo
                                 */
                                if ($atributo == "fk_id_codigo_peticion")
                                    $fk_id_codigo_peticion = $record[$atributo];
                                if ($atributo == "fk_id_oec")
                                    $fk_id_oec = $record[$atributo];
                            }
                            ...
                            if ($model->validate()) {
                                ...
                                //$codPeticion = $record['codigo_temporal'];
                                ...
                                /**
                                 * Primero verificamos si existe un registro con esa peticion y con ese estado si no existe creamos un nuevo registro peticion estado 
                                 */
                                $countpe = PeticionEstado010403::find()->where(['fk_id_peticion' => $model->getPrimaryKey(), 'fk_id_estado' => $stateQuery->id_estado])->count();
                                if ($countpe == 0) {
                                    PeticionEstado010403::updateAll(['activo_peticion_estado' => 0], ['fk_id_peticion' => $model->getPrimaryKey()]);
                                ...
                                foreach ($estadoNots as $estadoNot) {
                                    ...
                                    //$mensaje = str_replace("{codigo_peticion}",$codPeticion->codigo_temporal,$estadoNot->mensaje_estado_notificacion);
                                    ...
                                    $usuarios = Usuario000101::find()
                                            ...
                                            ->where(['fk_id_usuario_tipo' => $estadoNot->fk_id_usuario_tipo, 'fk_id_oec' => $fk_id_oec])
                                            ->all();
                                ...
                                for ($i = 0; $i < sizeof($transQuery); $i++) {
                                    ...
                                    for ($j = 0; $j < sizeof($transQuery[$i]['fkIdAccions']); $j++) {
                                        ...
                                        $countpa = PeticionAccion010301::find()->where(['fk_id_peticion' => $model->getPrimaryKey(), 'fk_id_accion' => $transQuery[$i]['fkIdAccions'][$j]['id_accion'], 'fk_id_transicion' => $transQuery[$i]['id_transicion']])->count();
                                        if ($countpa == 0) {
                                            $audi = new LogSistema030003();
