...
namespace app\controllers\AlcanceAcreditacionInsp041303;
use yii\base\Action;
...
class UpdateAction extends Action
{
    public function run()
    {
        ...
        if ($error == "") {
			...
			$model=new AlcanceAcreditacionInsp041303();
    	        	...
    	        	try {
						...
						if (sizeof($listDifAtriP) == 0) {
							...
							$listDiffRel = array_diff($listObjRels,$model->getListHasMany());
                                                        $pk=$model->getNamePk();
							$model = AlcanceAcreditacionInsp041303::findOne($record[$pk[0]]);
							if ($model) {
                                            ...
                                            if ($record[$listObjRel]){
                                                ...
                                                foreach ($record[$listObjRel] as $subCampo):
                                                    ...
                                                    if(substr($listObjRel2,-2) != "02"){
	                                                    ...
	                                                    if ($sw==0) {
	                                                        $objRel::deleteAll("fk_id_alcance_acreditacion_insp = ".$model->getPrimaryKey());
	                                                        $sw=1;
	                                                    }
	                                                }
                                                    
                                                    foreach($objRel->atributes() as $campo):	
                                                        if ($campo == 'fk_id_alcance_acreditacion_insp')
                                                            $objRel->$campo=$record['id_alcance_acreditacion_insp'];
                                                        else {
