...
namespace app\controllers\UsuarioTipoPrivilegio000202;
use yii\base\Action;
    	        	...
    	        	try {
						...
						if (sizeof($listDifAtriP) == 0) {
							...
							$listDiffRel = array_diff($listObjRels,$model->getListHasMany());
                                                        $pk=$model->getNamePk();
							$model = UsuarioTipoPrivilegio000202::findOne($record[$pk[0]]);
							if ($model) {
                                        ...
                                        if (in_array($listObjRel2,$model->getListHasMany())) {		
                                            ...
                                            if ($record[$listObjRel]){
                                                ...
                                                foreach ($record[$listObjRel] as $subCampo):
                                                    ...
                                                    if ($sw==0) {
                                                        $objRel::deleteAll("fk_fk_id_usuario_tipo = ".$model->getPrimaryKey());
                                                        $sw=1;
                                                    }
                                                    
                                                    foreach($objRel->atributes() as $campo):	
                                                        if ($campo == 'fk_fk_id_usuario_tipo')
                                                            $objRel->$campo=$record['fk_id_usuario_tipo'];
                                                        else {
