		...
		if ($error == "") {
			try {
				if (isset($_GET['sort'])) {
					...
					$propertySort	= "id_peticion_archivo_tipo";
					$directionSort	= "asc";
				}
                $condiQuery = "";
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = PeticionArchivoTipo010404::find()
                    							->distinct(true)
												->asArray()
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = PeticionArchivoTipo010404::find()
                                				->distinct(true)
                                                ->asArray()
                        ...
                        if ($sw == 0) {
                        	...
                        	$models = PeticionArchivoTipo010404::find()
                            					->distinct(true)
                                                ->asArray()
                                                ->where(['IN','id_peticion_archivo_tipo',$listSubQuery[0]])
                                                ->andWhere(['IN','id_peticion_archivo_tipo',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = PeticionArchivoTipo010404::find()
                            					->distinct(true)
