...
namespace app\controllers\AccionObservacion010203;
use yii\base\Action;
...
class UpdateAction extends Action
{
    public function run()
    {
        ...
        if ($error == "") {
			...
			$model=new AccionObservacion010203();
    	        	...
    	        	try {
						...
						if (sizeof($listDifAtriP) == 0) {
							...
							$listDiffRel = array_diff($listObjRels,$model->getListHasMany());
                                                        $pk=$model->getNamePk();
							$model = AccionObservacion010203::findOne($record[$pk[0]]);
							if ($model) {
                                        ...
                                        if (in_array($listObjRel2,$model->getListHasMany())) {		
                                            ...
                                            if ($record[$listObjRel]){
                                                ...
                                                foreach ($record[$listObjRel] as $subCampo):
                                                    ...
                                                    if ($sw==0) {
                                                        $objRel::deleteAll("fk_id_accion_observacion = ".$model->getPrimaryKey());
                                                        $sw=1;
                                                    }
                                                    
                                                    foreach($objRel->atributes() as $campo):	
                                                        if ($campo == 'fk_id_accion_observacion')
                                                            $objRel->$campo=$record['id_accion_observacion'];
                                                        else {
