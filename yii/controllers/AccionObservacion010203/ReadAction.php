		...
		if ($error == "") {
			try {
				if (isset($_GET['sort'])) {
					...
					$propertySort	= "id_accion_observacion";
					$directionSort	= "asc";
				}
                $condiQuery = "";
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = AccionObservacion010203::find()
                    							->distinct(true)
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = AccionObservacion010203::find()
                                				->distinct(true)
                                                ->with('fkIdAccion','fkIdUsuarioTipo')
                                                ->asArray()
												->joinWith('fkIdAccion')
                        ...
                        if ($sw == 0) {
							...
							->joinWith('fkIdAccion')
							->joinWith('fkIdUsuarioTipo')
                                                ->where(['IN','id_accion_observacion',$listSubQuery[0]])
                                                ->andWhere(['IN','id_accion_observacion',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = AccionObservacion010203::find()
                            					->distinct(true)
