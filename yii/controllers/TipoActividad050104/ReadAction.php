		...
		if ($error == "") {
			try {
				if (isset($_GET['sort'])) {
					...
					$propertySort	= "id_tipo_actividad";
					$directionSort	= "asc";
				}
                $condiQuery = "";
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = TipoActividad050104::find()
                    							->distinct(true)
												->asArray()
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = TipoActividad050104::find()
                                				->distinct(true)
                                                ->asArray()
                        ...
                        if ($sw == 0) {
                        	...
                        	$models = TipoActividad050104::find()
                            					->distinct(true)
                                                ->asArray()
                                                ->where(['IN','id_tipo_actividad',$listSubQuery[0]])
                                                ->andWhere(['IN','id_tipo_actividad',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = TipoActividad050104::find()
                            					->distinct(true)
