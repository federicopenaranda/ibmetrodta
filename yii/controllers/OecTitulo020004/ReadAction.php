...
namespace app\controllers\OecTitulo020004;
use yii\base\Action;
		...
		if ($error == "") {
			try {
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = OecTitulo020004::find()
                    							->distinct(true)
												->with('oecContacto020003s')
												->asArray()
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = OecTitulo020004::find()
                                				->distinct(true)
                                                ->with('oecContacto020003s')
                                                ->asArray()
												->joinWith('oecContacto020003s')
                        ...
                        if ($sw == 0) {
                        	...
                        	$models = OecTitulo020004::find()
                                                ...
                                                ->asArray()
							->joinWith('oecContacto020003s')
                                                ->where(['IN','id_oec_titulo',$listSubQuery[0]])
                                                ->andWhere(['IN','id_oec_titulo',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = OecTitulo020004::find()
                            					->distinct(true)
