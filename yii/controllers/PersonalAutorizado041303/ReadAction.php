...
namespace app\controllers\PersonalAutorizado041303;
use yii\base\Action;
...
class ReadAction extends Action
{
    public function run()
    {
		...
		$model = new PersonalAutorizado041303;
		$error="";
		...
		if ($error == "") {
			try {
				if (isset($_GET['sort'])) {
					...
					$propertySort	= "id_personal_autorizado";
					$directionSort	= "asc";
				}
                $condiQuery = "";
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = PersonalAutorizado041303::find()
                    							->distinct(true)
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = PersonalAutorizado041303::find()
                                				->distinct(true)
                                                ->with('fkIdAnexoConvenio')
                                                ->asArray()
												->joinWith('fkIdAnexoConvenio')
                        ...
                        if ($sw == 0) {
                        	...
                        	$models = PersonalAutorizado041303::find()
                                                ...
                                                ->asArray()
							->joinWith('fkIdAnexoConvenio')
                                                ->where(['IN','id_personal_autorizado',$listSubQuery[0]])
                                                ->andWhere(['IN','id_personal_autorizado',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = PersonalAutorizado041303::find()
                            					->distinct(true)
