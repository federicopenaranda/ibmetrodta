...
namespace app\controllers\OecAcreditacionSolicitada020003;
use yii\base\Action;
...
class ReadAction extends Action
{
    public function run()
    {
		...
		$model = new OecAcreditacionSolicitada020003;
		$error="";
		...
		if ($error == "") {
			try {
				if (isset($_GET['sort'])) {
					...
					$propertySort	= "id_oec_acreditacion_solicitada";
					$directionSort	= "asc";
				}
                $condiQuery = "";
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = OecAcreditacionSolicitada020003::find()
                    							->distinct(true)
												->with('fkIdOec')
												->asArray()
												->joinWith('fkIdOec')
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = OecAcreditacionSolicitada020003::find()
                                				->distinct(true)
                                                ->with('fkIdOec')
                                                ->asArray()
												->joinWith('fkIdOec')
                        ...
                        if ($sw == 0) {
                        	...
                        	$models = OecAcreditacionSolicitada020003::find()
                                                ...
                                                ->asArray()
							->joinWith('fkIdOec')
                                                ->where(['IN','id_oec_acreditacion_solicitada',$listSubQuery[0]])
                                                ->andWhere(['IN','id_oec_acreditacion_solicitada',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = OecAcreditacionSolicitada020003::find()
                            					->distinct(true)
