...
namespace app\controllers\Gestion050104;
use yii\base\Action;
    	        	...
    	        	try {
						...
						if (sizeof($listDifAtriP) == 0) {
							...
							$listDiffRel = array_diff($listObjRels,$model->getListHasMany());
                                                        $pk=$model->getNamePk();
							$model = Gestion050104::findOne($record[$pk[0]]);
							if ($model) {
                                        ...
                                        if (in_array($listObjRel2,$model->getListHasMany())) {		
                                            ...
                                            if ($record[$listObjRel]){
                                                ...
                                                foreach ($record[$listObjRel] as $subCampo):
                                                    ...
                                                    if ($sw==0) {
                                                        $objRel::deleteAll("fk_id_gestion = ".$model->getPrimaryKey());
                                                        $sw=1;
                                                    }
                                                    
                                                    foreach($objRel->atributes() as $campo):	
                                                        if ($campo == 'fk_id_gestion')
                                                            $objRel->$campo=$record['id_gestion'];
                                                        else {
