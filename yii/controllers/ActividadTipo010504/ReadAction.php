...
namespace app\controllers\ActividadTipo010504;
use yii\base\Action;
		...
		if ($error == "") {
			try {
				if (isset($_GET['sort'])) {
					...
					$propertySort	= "id_actividad_tipo";
					$directionSort	= "asc";
				}
                $condiQuery = "";
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = ActividadTipo010504::find()
                    							->distinct(true)
												->asArray()
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = ActividadTipo010504::find()
                                				->distinct(true)
                                                ->asArray()
                        ...
                        if ($sw == 0) {
                        	...
                        	$models = ActividadTipo010504::find()
                            					->distinct(true)
                                                ->asArray()
                                                ->where(['IN','id_actividad_tipo',$listSubQuery[0]])
                                                ->andWhere(['IN','id_actividad_tipo',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = ActividadTipo010504::find()
                            					->distinct(true)
