...
namespace app\controllers\AlcanceAcreditacionLab041303;
use yii\base\Action;
...
class UpdateAction extends Action
{
    public function run()
    {
        ...
        if ($error == "") {
			...
			$model=new AlcanceAcreditacionLab041303();
    	        	...
    	        	try {
						...
						if (sizeof($listDifAtriP) == 0) {
							...
							$listDiffRel = array_diff($listObjRels,$model->getListHasMany());
                                                        $pk=$model->getNamePk();
							$model = AlcanceAcreditacionLab041303::findOne($record[$pk[0]]);
							if ($model) {
                                            ...
                                            if ($record[$listObjRel]){
                                                ...
                                                foreach ($record[$listObjRel] as $subCampo):
                                                    ...
                                                    if(substr($listObjRel2,-2) != "02"){
														$objRel=$objRel::find()->where([$objRel->getNamePk() => $record[$objRel->getNamePk()]])->one();												
													}
                                                    if ($sw==0) {
                                                        $objRel::deleteAll("fk_id_alcance_acreditacion_lab = ".$model->getPrimaryKey());
                                                        $sw=1;
                                                    }
                                                    
                                                    foreach($objRel->atributes() as $campo):	
                                                        if ($campo == 'fk_id_alcance_acreditacion_lab')
                                                            $objRel->$campo=$record['id_alcance_acreditacion_lab'];
                                                        else {
