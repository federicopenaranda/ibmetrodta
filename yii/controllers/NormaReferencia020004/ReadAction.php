...
namespace app\controllers\NormaReferencia020004;
use yii\base\Action;
...
class ReadAction extends Action
{
    public function run()
    {
		...
		$model = new NormaReferencia020004;
		$error="";
		...
		if ($error == "") {
			try {
				if (isset($_GET['sort'])) {
					...
					$propertySort	= "id_norma_referencia";
					$directionSort	= "asc";
				}
                $condiQuery = "";
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = NormaReferencia020004::find()
                    							->distinct(true)
												->with('oec020001s')
												->asArray()
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = NormaReferencia020004::find()
                                				->distinct(true)
                                                ->with('oec020001s')
                                                ->asArray()
												->joinWith('oec020001s')
                        ...
                        if ($sw == 0) {
                        	...
                        	$models = NormaReferencia020004::find()
                                                ...
                                                ->asArray()
							->joinWith('oec020001s')
                                                ->where(['IN','id_norma_referencia',$listSubQuery[0]])
                                                ->andWhere(['IN','id_norma_referencia',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = NormaReferencia020004::find()
                            					->distinct(true)
