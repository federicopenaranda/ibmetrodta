		...
		if ($error == "") {
			try {
				if (isset($_GET['sort'])) {
					...
					$propertySort	= "id_no_conformidad";
					$directionSort	= "asc";
				}
                $condiQuery = "";
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = NoConformidad040701::find()
                    							->distinct(true)
												->with('fkIdPeticion')
												->asArray()
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = NoConformidad040701::find()
                                				->distinct(true)
                                                ->with('fkIdPeticion')
                                                ->asArray()
												->joinWith('fkIdPeticion')
                        ...
                        if ($sw == 0) {
                        	...
                        	$models = NoConformidad040701::find()
                                                ...
                                                ->asArray()
							->joinWith('fkIdPeticion')
                                                ->where(['IN','id_no_conformidad',$listSubQuery[0]])
                                                ->andWhere(['IN','id_no_conformidad',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = NoConformidad040701::find()
                            					->distinct(true)
