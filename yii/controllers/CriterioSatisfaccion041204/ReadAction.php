...
namespace app\controllers\CriterioSatisfaccion041204;
use yii\base\Action;
...
class ReadAction extends Action
{
    public function run()
    {
		...
		$model = new CriterioSatisfaccion041204;
		$error="";
		...
		if ($error == "") {
			try {
				if (isset($_GET['sort'])) {
					...
					$propertySort	= "id_criterio_satisfaccion";
					$directionSort	= "asc";
				}
                $condiQuery = "";
                ...
                if (isset($_GET['query']) && $_GET['query'] != "" && $condiQuery != "") {
                    $models = CriterioSatisfaccion041204::find()
                    							->distinct(true)
												->asArray()
                    ...
                    if (isset($_GET['filter']) && $_GET['filter']!='') {
                        ...
                        foreach($filtros as $filtro):
                            ...
                            if (in_array($filtro['property'],$listManyMany)) {
                                ...
                                $listSubQuery[] = CriterioSatisfaccion041204::find()
                                				->distinct(true)
                                                ->asArray()
                        ...
                        if ($sw == 0) {
                        	...
                        	$models = CriterioSatisfaccion041204::find()
                            					->distinct(true)
                                                ->asArray()
                                                ->where(['IN','id_criterio_satisfaccion',$listSubQuery[0]])
                                                ->andWhere(['IN','id_criterio_satisfaccion',$listSubQuery[sizeof($listSubQuery)-1]])
                            					->all();
                            $total = sizeof($models);
                            $models = CriterioSatisfaccion041204::find()
                            					->distinct(true)
