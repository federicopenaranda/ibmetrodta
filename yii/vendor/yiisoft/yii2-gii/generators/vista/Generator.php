...
namespace yii\gii\generators\vista;
...
class Generator extends \yii\gii\Generator
{
    ...
    {
        return 'Este generador genera las vista de cada modelo de la base de datos.';
    }
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
			...
			#add
			$files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . strtolower($className) . '/create.php',
                $this->render('create.php', $params)
            );
			$files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . strtolower($className) . '/read.php',
                $this->render('read.php', $params)
            );
			$files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . strtolower($className) . '/update.php',
                $this->render('update.php', $params)
            );
			$files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . strtolower($className) . '/delete.php',
                $this->render('delete.php', $params)
            );

            if ($params["className"] == "Usuario000101") {
                $files[] = new CodeFile(
                        Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) .'/'. strtolower($className).'/login.php',
                        $this->render('logout.php', $params)
                    );
                $files[] = new CodeFile(
                        Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) .'/'. strtolower($className).'/logout.php',
                        $this->render('logout.php', $params)
                    );
            }
			...
			foreach($table->columns as $column):
				if ($column->comment == "file" && $sw == 1) {
					$params['file'] = 'file';
					$files[] = new CodeFile(
						Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . strtolower($className) . '/upload.php',
		...
		$files[] = new CodeFile(
					Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' .'permisos/read.php',
					$this->render('read.php', $params)
				);
