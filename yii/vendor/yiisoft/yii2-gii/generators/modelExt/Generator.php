...
namespace yii\gii\generators\modelExt;
...
use yii\gii\generators\modelExt\Catequil;
...
class Generator extends \yii\gii\Generator
{
    ...
    {
        return 'Model JSExt4.2 Generator';
    }
    ...
    {
        return 'Este generador genera los Modelos de JSExt4.';
    }
    ...
    {
        return ['modelo_1.php','modelo_4.php','modelo_5.php','Secundaria_11.php'];
    }
        ...
        foreach ($this->getTableNames() as $tableName) {
			...
			*/
			if ($listCodigo[2] == "01") {
				...
				foreach($model->getListHasMany() as $nomSubModel):
					...
					$subParams = ['className'=>$params['className'],'subModelClass'=>$nomSubModel];
					...
					if ($listCodigo[0] == $listSubCodigo[0] && $listCodigo[1] == $listSubCodigo[1]) { 						
						...
						if ($listSubCodigo[2] == 3 ) {
                            ...
                            $files[] = new CodeFile(
								Yii::getAlias('@app'.'/app/Patron1/model').'/'.getNameTableSinCod($nomSubModel).'.js',
								$this->render('Secundaria_11.php', $listSecundaria[$nomSubModel])
							);
						}
					}
				endforeach;
				...
				$files[]=new CodeFile(
					Yii::getAlias('@app'.'/app/Patron1/model').'/'.getNameTableSinCod($className).'.js',
					$this->render('/modelo_1.php', $params)
				);
			}
			/**
			* PATRON 4 MODELO (Referencial) 
			* Se genera para las 2 opcones (a,b) ya que tienen el mismo codigo
			*/
			if ($listCodigo[2] == "04") {
				...
				$files[] = new CodeFile(
					Yii::getAlias('@app'.'/app/Patron4/model').'/'.$modulo3.'/'.getNameTableSinCod($className).'.js',
					$this->render('/modelo_4.php', $params)
				);
			}
			/**
			* PATRON 5 MODELO
			*/
			if ($listCodigo[2] == "05") {
				...
				foreach($model->getListHasMany() as $nomSubModel):
					...
					if ($listCodigo[0] == $listSubCodigo[0] && $listCodigo[1] == $listSubCodigo[1]) { 						
						...
						if ($listSubCodigo[2] == 3 ) {
                            ...
                            $files[] = new CodeFile(
								Yii::getAlias('@app'.'/app/Patron5/model').'/'.getNameTableSinCod($nomSubModel).'.js',
								$this->render('Secundaria_11.php', $listSecundaria[$nomSubModel])
							);
						}
					}
				...
				$files[] = new CodeFile(
					Yii::getAlias('@app'.'/app/Patron5/model').'/'.getNameTableSinCod($className).'.js',
					$this->render('/modelo_5.php', $params)
				);
			}
        }
