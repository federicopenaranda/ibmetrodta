...
namespace yii\gii\generators\controlador;
...
class Generator extends \yii\gii\Generator
{
    ...
    {
        return 'Este generador genera controladores con sus respectivas acciones.';
    }
    ...
    {
        #return ['model.php'];
		return ['model.php','create.php','read.php','update.php','delete.php','upload.php'];
    }
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
			...
			foreach($model->getListHasOne() as $modOne):
				$objOne = getPath($modOne);
				$codigo = divideCodigo($model->tableName());
				#echo $className." - ".$modOne."/".$codigo[2];
				if ($codigo[2] != '02' /*&& strpos($display,$objOne->getDisplay()!==FALSE)*/) {
					if ($objOne->getDisplay()=='') {
						$display.=$modOne.' - ';
					}
				}
			endforeach;
			...
			#if ($listCodigo[2]!='2'){
				$files[] = new CodeFile(
					Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . ucwords(strtolower($className)) . 'Controller.php',
					$this->render('model.php', $params)
				);	
				//add
				$files[] = new CodeFile(
					Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '/CreateAction.php',
					$this->render('create.php', $params)
				);
				$files[] = new CodeFile(
					Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '/ReadAction.php',
					$this->render('read.php', $params)
				);
				$files[] = new CodeFile(
					Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '/UpdateAction.php',
					$this->render('update.php', $params)
				);
				$files[] = new CodeFile(
					Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '/DeleteAction.php',
					$this->render('delete.php', $params)
				);
				if ($className == "Usuario000101"){
					$files[] = new CodeFile(
						Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '/LoginAction.php',
						$this->render('login.php', $params)
					);
					$files[] = new CodeFile(
						Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '/LogoutAction.php',
						$this->render('logout.php', $params)
					);
					$files[] = new CodeFile(
						Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '/ListaPrivilegiosUsuarioAction.php',
						$this->render('listaPrivilegiosUsuario.php', $params)
					);
				}
				//verfica si tiene campos archivo
				$table=$db->getSchema()->getTableSchema($tableName);
				foreach($table->columns as $column):
					if($column->comment == "file"){
						$params['file'] = 'file';
						$params['nameFile']=ucwords(str_replace("_","",$column->name));
						$files[] = new CodeFile(
							Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '/Upload'.str_replace("_","",ucwords($column->name)).'Action.php',
							$this->render('upload.php', $params)
						);
						$files[] = new CodeFile(
							Yii::getAlias('@app'. "/web/".$className.'/'.str_replace("_","",ucwords($column->name)).'/leeme.txt'),
							$this->render('upload.php', $params)
						);
						$files[] = new CodeFile(
							Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '/Download'.str_replace("_","",ucwords($column->name)).'Action.php',
							$this->render('download.php', $params)
						);
					}
				endforeach;
				//veririfica los campos unicos
				$db = Yii::$app->get($this->db, false);
				$table = $db->getSchema()->getTableSchema($tableName);
				$uniqueIndexes = $db->getSchema()->findUniqueIndexes($table);
				$params['uniqueIndexes'] = $uniqueIndexes;
				$files[] = new CodeFile(
					Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '/CheckUniqueAction.php',
					$this->render('checkunique.php', $params)
				);
			$nomTab[]=$className;
		...
		$param=[
				'modelClass'=>$nomTab,];
		if (in_array("Usuario000101",$nomTab)){
			$files[] = new CodeFile(
					Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/Usuario000101/InsertAction.php',
					$this->render('insert.php', $param)
				);
		}
		
		$files[] = new CodeFile(
					Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/PermisosController.php',
					$this->render('permisos.php', $param)
				);
		if($display!='')
			echo "<script>alert('TABLAS QUE LE FALTAN DISPLAY: ".$display."')</script>";
