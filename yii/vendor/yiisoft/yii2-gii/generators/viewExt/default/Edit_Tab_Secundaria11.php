...
Ext.define('<?= $path;?>.view.<?= $modulo; ?>.<?= getNameTableSinCod($className); ?>.edit.tab.<?= getNameTableSinCod($subModelClass); ?>', {
    ...
    initComponent: function() {
        ...
        Ext.applyIf(me, {
            items: [
                {
                    xtype: '<?= strtolower($modulo); ?>.<?= strtolower(getNameTableSinCod($subModelClass)); ?>lista',
                    title: 'Secundaria 1',
                    iconCls: 'icon_<?= $modulo; ?>',
                    store: Ext.create( '<?= $path;?>.store.<?= $modulo; ?>.<?= strtolower(getNameTableSinCod($subModelClass)); ?>', {
                        pageSize: 10
                    })
                }
            ]
