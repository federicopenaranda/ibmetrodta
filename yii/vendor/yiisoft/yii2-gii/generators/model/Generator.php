...
class Generator extends \yii\gii\Generator
{
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
			...
			if ($className == "Usuario000101") {
				...
				$files[] = new CodeFile(
                	Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . 'LoginForm.php',
                	$this->render('modelForm.php', $params)
            	);
				$files[] = new CodeFile(
                	Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '.php',
                	$this->render('modelusuario.php', $params)
            	);
			} else {
				//se quedo el original la de abajo
            	$files[] = new CodeFile(
                	Yii::getAlias('@' . str_replace('\\', '/', $this->ns)) . '/' . $className . '.php',
			...
			//add
			$listClassName[] = $this->generateClassName($tableName);
        }
		//addd
		$files[] = new CodeFile(
			Yii::getAlias('@' . str_replace('\\', '/','app\vendor\yiisoft\yii2-gii\generators\controlador\default')) . '/instancia.php',
			$this->render('instancia.php',["nameClass"=>$listClassName])
        );
        return $files;
    }
    ...
    public function generateRules($table)
    {
        ...
        foreach ($table->columns as $column) {
			...
			if (!$column->allowNull && $column->defaultValue === null && strpos($column->name,"fecha_creacion")===FALSE) {
     ...
     */
    public function checkPivotTable($table) {
