...
namespace yii\gii\generators\store;
...
#use yii\gii\generators\store\Catequil;
...
class Generator extends \yii\gii\Generator
{
    ...
    {
        return 'Store JSExt4.2 Generator';
    }
    ...
    {
        return 'Este generador genera lo stores de JSExt4.';
    }
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
			...
			*/		
			if ($listCodigo[2] == "01") {
				...
				foreach($model->getListHasMany() as $nomSubModel):
					...
					if ($listCodigo[0] == $listSubCodigo[0] && $listCodigo[1] == $listSubCodigo[1]) { 						
						...
						if ($listSubCodigo[2] == 3 ) {
                            ...
                            $files[] = new CodeFile(
								Yii::getAlias('@app'.'/app/Patron1/store').'/'.getNameTableSinCod($nomSubModel).'.js',
				...
				$files[]=new CodeFile(
					Yii::getAlias('@app'.'/app/Patron1/store').'/'.getNameTableSinCod($className).'.js',
					$this->render('/Patron1_store.php', $params)
				);
				
				$listPathArch[] = $subParams['modulo'].'.'.getNameTableSinCod($className);
			}
			/**
			* PATRON 4 REFERENCIAL 
			* Se generan para ambos stores (a,b) ya que tienen el mismo codigo  
			*/
			...
			if ($listCodigo[2] == "04") {
				...
				$files[]=new CodeFile(
					Yii::getAlias('@app'.'/app/Patron4/store').'/'.$modulo3.'/'.getNameTableSinCod($className).'.js',
					$this->render('/Patron4_store.php', $params)
				);
			...
			*/
			if ($listCodigo[2] == "05") {
				...
				foreach($model->getListHasMany() as $nomSubModel):
					...
					if ($listCodigo[0] == $listSubCodigo[0] && $listCodigo[1] == $listSubCodigo[1]) { 						
						...
						if ($listSubCodigo[2] == 3 ) {
							...
							$listSubModMen[]=$nomSubModel;
                            $files[] = new CodeFile(
								Yii::getAlias('@app'.'/app/Patron5/store').'/'.getNameTableSinCod($nomSubModel).'.js',
								$this->render('Secundaria_11.php', $subParams)
							);
				...
				$files[]=new CodeFile(
					Yii::getAlias('@app'.'/app/Patron5/store').'/'.getNameTableSinCod($className).'.js',
					$this->render('/Patron5_store.php', $params)
				);
		...
		$files[]=new CodeFile(
				Yii::getAlias('@app'.'/app').'/'.'ArchStore.js',
				$this->render('/ArchStore.php', $listRutas)
		);
		
		if ($module!="") {
			echo "<script>alert('TABLAS QUE LE FALTAN LOS MODULOS: ".$module."')</script>";
		}
